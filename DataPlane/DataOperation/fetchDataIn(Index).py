#!/usr/bin/env python3
"""
VikingDB - Fetch Data by ID in Index (Data Plane)
Official endpoint: /api/vikingdb/data/fetch_in_index
"""
...
def call_dataplane(body: dict, path: str):
    ...
    # No Action/Version in query for this endpoint
    query = {}
...
# ────────────────────────────────────────────────
# Fetch by ID in Index – Official Endpoint
# ────────────────────────────────────────────────
def fetch_by_id_in_index():
    print("\n=== Fetch Records by ID in Index (Official Endpoint) ===")

    collection = input("Collection name [ImageCollection]: ").strip() or "ImageCollection"
    index_name = input("Index name [idx_hnsw_1]: ").strip() or "idx_hnsw_1"

    print("\nExample IDs (from your multimodal search):")
    ...
    try:
        resp = call_dataplane(
            body=body,
            path="/api/vikingdb/data/fetch_in_index"   # Official path
        )
...
if __name__ == "__main__":
    print("VikingDB - Fetch by ID in Index (Official Endpoint)")
    print("==================================================\n")

    while True:
        print("\nOptions:")
        print("  1. Fetch records by ID in Index")
        print("  0. Exit")
