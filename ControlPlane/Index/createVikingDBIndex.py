...
def create_index(client):
    body = {
        "ProjectName": PROJECT_NAME,          # optional
        "CollectionName": COLLECTION_NAME,    # or use ResourceId instead
        "IndexName": "testing_only",
        "Description": "HNSW cosine index",
        "ShardPolicy": "auto",                # or "custom"
        # "ShardCount": 1,                    # only used when ShardPolicy="custom"

        "VectorIndex": {
            "IndexType": "hnsw",              # allowed: hnsw, hnsw_hybrid, flat, diskann :contentReference[oaicite:2]{index=2}
            "Distance": "cosine",             # allowed: ip, l2, cosine :contentReference[oaicite:3]{index=3}
            "Quant": "float",                 # matches doc example style :contentReference[oaicite:4]{index=4}

            # Optional HNSW tuning params exist (names shown in doc);
            # you can start without them and add later if needed. :contentReference[oaicite:5]{index=5}
            # "HnswM": 16,
            # "HnswCef": 200,
            # "HnswSef": 200,
        }
